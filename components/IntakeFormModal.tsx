...
const IntakeFormModal: React.FC<IntakeFormModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
     ...
     * Simulating the secure dispatch of intake data to the support node.
     */
    console.log('[PROTOCOL INITIATED]: Dispatching intake submission to support@blackstarva.com');
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 sm:p-6 bg-black/90 backdrop-blur-xl animate-fade-in">
      <div className={`glass w-full rounded-[3rem] border-white/10 shadow-2xl overflow-hidden flex flex-col transition-all duration-700 ease-in-out max-w-2xl ${isSuccess ? 'max-h-[500px]' : 'max-h-[90vh]'}`}>
        
        {/* Success View */}
        {isSuccess ? (
          <div className="flex-grow flex flex-col items-center justify-center p-12 text-center animate-cartoon-pop">
            <div className="w-24 h-24 bg-[#ab7e31]/20 rounded-full flex items-center justify-center mb-8 border border-[#ab7e31]/30 shadow-[0_0_40px_rgba(171,126,49,0.2)]">
            ...
            <h2 className="text-4xl font-black logo-font text-white mb-6 uppercase tracking-tighter">
              Mission <br /><span className="text-[#ab7e31] italic font-light lowercase">Synchronized.</span>
            </h2>
            <p className="text-gray-400 font-light leading-relaxed mb-10 max-w-md">
              Your intake profile has been dispatched to <span className="text-white font-bold">support@blackstarva.com</span>. Our lead strategists will review your operational drag and reach out within 4 business hours.
            </p>
            <button 
              onClick={onClose}
                  ...
                  {step < totalSteps ? (
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        <>Submit <i className="fas fa-paper-plane ml-3"></i></>
                      )}
                    </button>
                  )}
                </div>
